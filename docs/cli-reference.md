...
| `setup`                                | One-command onboarding: report the environment, optionally install Docker through the platform package manager after explicit confirmation, then optionally run a self-contained fail-to-pass demo. Flags: `--check` (report only), `--demo`, `--demo-dir <dir>`. |
