...
The pull-request fixture `.patchproof.yml` declares the production Docker backend. The checked-in `local.patchproof.yml` is a development copy that opts into the unsafe local process backend; its trusted policy allows the local run shown above. The CLI flags shown above also explicitly select the local backend.
