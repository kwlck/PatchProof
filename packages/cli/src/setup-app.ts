...
export function renderEnvFile(credentials: AppCredentials): string {
  return [
    ...
    '# Keep this file private; it grants write access to repositories the App can reach.',
...
export async function runSetupApp(args: ParsedArgs): Promise<number> {
  ...
  try {
    await lstat(envPath);
    throw new Error(`Credential file already exists: ${envPath}; choose a new --env-file`);
  } catch (error) {
  ...
  await writeFile(envPath, renderEnvFile(credentials), {
    encoding: 'utf8',
    mode: 0o600,
    flag: 'wx',
  });
  console.log(
    [
      ...
      '',
      'The App webhook is inactive until you set its public HTTPS URL and enable it.',
      'In App settings, set Webhook URL to https://<your-host>/webhooks/github',
      'and use PATCHPROOF_WEBHOOK_SECRET from the env file as its secret.',
      '',
  ...
  console.log(
    [
      ...
      '',
      'Deploy the GitHub App from a PatchProof source checkout (the standalone CLI does not bundle it).',
      `Load credentials from ${envPath} into both processes. Configure`,
      'PATCHPROOF_APPROVED_DOCKER_IMAGES with a digest-pinned approved image.',
      'Then start both processes from the built source checkout:',
      '  pnpm --filter @patchproof/github-app start:webhook',
      '  pnpm --filter @patchproof/github-app start:worker',
      'See docs/deployment.md for the public webhook URL and worker requirements.',
    ].join(NEWLINE),
  );
  return 0;
}
